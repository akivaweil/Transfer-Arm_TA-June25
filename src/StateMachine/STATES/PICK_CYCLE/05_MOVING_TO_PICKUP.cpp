...
const float RETURN_X_PICKUP_INCHES = 1.0;                // X position to move to pickup (inches) - updated from original
...
// Handles the return sequence: turn off Stage 2 signal, reset servo, move directly to pickup
// When complete, transitions back to being at pickup position (skipping homing)

bool handleMovingToPickup() {
  static int returnStep = 0;
  
  switch(returnStep) {
    case 0:  // Signal Stage 2 and move X directly to pickup - moving right
      ...
      if (xStepper) {
        ...
        xStepper->moveTo((int32_t)(RETURN_X_PICKUP_INCHES * STEPS_PER_INCH));  // Move directly to pickup position
      }
      returnStep = 1;
      break;
      
    case 1:  // Wait for X to reach pickup position - moving right
