...
const float X_PRE_HOMING_POS = X_PRE_HOMING_POS_INCHES * STEPS_PER_INCH;  // Pre-homing position in steps
...
const float SERVO_PICKUP_POS = 20.0;  // Servo pickup position (in degrees)
const float SERVO_TRAVEL_POS = 0.0;   // Servo position for travel after pickup (in degrees)
const float SERVO_DROPOFF_POS = 92.0; // Servo dropoff position (90 degrees from pickup)
...
const unsigned long HOMING_TIMEOUT = 5000;   // Homing timeout (5 seconds)
...
const float Z_HOME_SPEED = 1500.0;      // Homing speed for Z-axis in steps per second 